...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  if(stat(filename,&filestat)==-1){
    printf("error from ReadFile:72 : %s\n",strerror(errno));
    return NULL;
  }
  ...
  if(!S_ISREG(filestat.st_mode)){
    printf("error from ReadFile:81: not a regular file.\n");
    return NULL;
  }
  ...
  if(fd==-1){
    printf("error from ReadFile:89 : %s\n",strerror(errno));
    return NULL;
  }
  ...
  while (left_to_read > 0) {
	numread=read(fd,buf,filestat.st_size);
        if(numread==-1){
		perror("error from ReadFile:115: ");
    		return NULL;
	}
	if(numread==0){
		break;
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  //
  while(isalpha(*curptr)==0&&(*curptr)!='\0'){
	curptr++;
  }
  if(*curptr=='\0') return;
  wordstart=curptr;
  
  //have to start from a character
  while(*wordstart!='\0'){
	while(isalpha(*curptr)){
		*curptr=tolower(*curptr);
		curptr++;
	}
	while(*curptr!='\0'&&!isalpha(*curptr)){
		*curptr='\0';
		curptr++;	
	}
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    newkeyvalue.key=hashKey;
    newkeyvalue.value=wp;
    retval=InsertHashTable(tab,newkeyvalue,&kv);
    Verify333(retval==1);
  }
}
