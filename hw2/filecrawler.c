...
static void HandleDir(char *dirpath, DIR *d, DocTable *doctable,
  ...
  while (1) {
    ...
    struct dirent *dirent = NULL, *entry=NULL;
    ...
    if (res == 0) {
      ...
      if(S_ISDIR(nextstat.st_mode)){
		DIR *childDir=opendir(newfile);
		HandleDir(newfile,childDir,doctable,index);
		closedir(childDir);
      }
    }
