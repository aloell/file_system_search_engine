...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    wds->word=word;
    HashTable docPos=AllocateHashTable(17);
    wds->docIDs=docPos;
    kv.key=wordkey;
    kv.value=wds;
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  char* q=query[0];
  wordkey = FNVHash64((unsigned char *) q, strlen(q));
  res=LookupHashTable(index, wordkey, &kv);
  if(res==0){
	FreeLinkedList(retlist,MINullFree);
	return NULL;
  }
  wds=kv.value;
  
  HTIter htiter=HashTableMakeIterator(wds->docIDs);
  if(htiter==NULL){
	fprintf(stderr,"no enough memory for htiterator!\n");
	exit(-1);
  }
  SearchResultPtr sr;
  DocID_t docid=0;
  HWSize_t rank=0;
  while(HTIteratorGet(htiter,&kv)){
	sr=(SearchResultPtr)malloc(sizeof(SearchResult));
	docid=kv.key;
	rank=NumElementsInLinkedList(kv.value);
	sr->docid=docid;
	sr->rank=rank;
        //printf("first word query rsts, docid:%u\n",(DocID_t)sr->docid);
	PushLinkedList(retlist,(LLPayload_t)sr);
        HTIteratorNext(htiter);
  }
  HTIteratorFree(htiter);
  //uint64_t kk=NumElementsInLinkedList(retlist);
  //printf("for word: %s,number of docs:%lu\n",query[0],kk);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    //uint64_t kkk=NumElementsInHashTable(wds->docIDs);
    //printf("for word: %s,number of docs:%lu\n",query[i],kkk);
    // STEP 6.
	...
	//printf("currently search for doci:%u, result:%d\n",sr->docid,res);
        if(res==0){
		free(sr);
