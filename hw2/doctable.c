...
void MyValueFreeFn (HTValue_t value){
	return;
}
...
void FreeDocTable(DocTable table) {
  ...
  // STEP 1.
  FreeHashTable(table->docid_to_docname, MyValueFreeFn);
  FreeHashTable(table->docname_to_docid, MyValueFreeFn);
  free(table);
}
...
DocID_t DTLookupDocumentName(DocTable table, char *docname) {
  ...
  if(res==1){
        //printf("doc_name:%s, doc_id:%d\n",docname,(DocID_t)kv.value);
