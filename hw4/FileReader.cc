...
bool FileReader::ReadFile(std::string *str) {
  ...
  // MISSING:
  if(!IsPathSafe(basedir_, fullfile)){return false;}
  uint32_t len=0;
  char *content=::ReadFile(fullfile.c_str(), &len);
  if(content==NULL) return false;
  std::unique_ptr<char,MallocDeleter<char>> uptr_content(content,MallocDeleter<char>{});
  string s_content(content,len);
  *str=s_content;
  return true;
}
