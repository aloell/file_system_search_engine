...
int main(int argc, char **argv) {
  ...
  list<string> index_files_l;
  for(int i=1;i<argc;i++){
	string index(argv[i]);
	index_files_l.push_back(index);
  }
  hw3::QueryProcessor qp(index_files_l);
  
  char delimit=' ';
  char delimit1='\n';
  string query;
  while (getline(cin,query,delimit1)) {
	stringstream query_s(query);
  	vector<string> query_v;
  	auto back_itr=back_inserter(query_v);
  	string query_token;
  	while(getline(query_s,query_token,delimit)){
		*back_itr=query_token;
		back_itr++;
  	}	
	vector<hw3::QueryProcessor::QueryResult> rst;
	rst=qp.ProcessQuery(query_v);
	cout<<"the query result for '"<<query<<"' is as follows:\n";
        for(auto& qr:rst){
		cout<<"docname: "<<qr.document_name<<" rank: "<<qr.rank<<endl;
	}
	cout<<"end of query result\n";
  }
