...
FileIndexReader::FileIndexReader(std::string filename,
  ...
  if (validate) {
    ...
    Verify333(fseek(file_, sizeof(IndexFileHeader), SEEK_SET)==0);
    while (left_to_read > 0) {
      // MISSING:
      if(left_to_read<512){
         fread(buf, 512, 1, file_);
         for(unsigned i=0;i<left_to_read;i++)
      		crcobj.FoldByteIntoCRC(buf[i]);
         break;
      }
      //printf("left to read:%d\n",left_to_read);
      HWSize_t readCnt=fread(buf, 512, 1, file_);
      //printf("read:%d\n",readCnt);
      //if(ferror(file_)) {printf("file read error\n");exit(-1);}
      //if(feof(file_)) {printf("file eof\n"); exit(-2);}
      left_to_read-=readCnt*512;
      for(unsigned i=0;i<readCnt*512;i++)
      	crcobj.FoldByteIntoCRC(buf[i]);
    }
