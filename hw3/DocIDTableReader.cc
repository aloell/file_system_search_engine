...
bool DocIDTableReader::LookupDocID(const DocID_t &docid,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    header.toHostFormat();
    // Is it a match?
    printf("offset: %d,doc id:%d, look for id:%d\n",next_offset,header.docid,docid);
    if (header.docid == docid) {
      ...
      // MISSING:
      std::list<DocPositionOffset_t> ret;
      docid_element_position elPos;
      for(unsigned i=0;i<header.num_positions;i++){
	 fread(&elPos, sizeof(docid_element_position), 1, file_);
         elPos.toHostFormat();
         ret.push_back(elPos.position);
      }
...
list<docid_element_header> DocIDTableReader::GetDocIDList() {
  ...
  for (HWSize_t i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file .
    // MISSING:
    int bucket_rec_offset=offset_+sizeof(header_)+i*sizeof(bucket_rec);
    int res=fseek(file_, bucket_rec_offset, SEEK_SET);
    Verify333(res==0);
    ...
    bucket_rec b_rec;
    // MISSING:
    fread(&b_rec, sizeof(bucket_rec), 1, file_);
    b_rec.toHostFormat();
    ...
    for (HWSize_t j = 0; j < b_rec.chain_len; j++) {
      ...
      // MISSING:
      fread(&ep, sizeof(element_position_rec), 1, file_);
      ep.toHostFormat();
      ...
      // MISSING:
      fread(&doc_el, sizeof(docid_element_header), 1, file_);
      doc_el.toHostFormat();
