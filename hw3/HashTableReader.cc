...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  // MISSING:
  int chain_len=b_rec.chain_len;
  int bucket_position=b_rec.bucket_position;
  element_position_rec* epr=new element_position_rec[chain_len];
  res=fseek(file_, bucket_position, SEEK_SET);
  Verify333(res==0);
  fread(epr, sizeof(element_position_rec), chain_len, file_);
  for(int i=0;i<chain_len;i++){
  	epr[i].toDiskFormat();
        retval.push_back(epr[i].element_position);
  }
  delete [] epr;
  // Return the list.
  return retval;
