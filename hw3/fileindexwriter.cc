...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  filesize += dtres;
  printf("doc table written:%d\n",dtres);
  // write the memindex using WriteMemIndex().
  // MISSING:
  filesize+=sizeof(IndexFileHeader);
  ...
  filesize += mires;
  printf("mem index table written:%d\n",mires);

  // write the header using WriteHeader().
  // MISSING:
  WriteHeader(f,dtres,mires);
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // MISSING:
  fwrite((char*)kv->value,slen_ho,1,f);
  uint32_t totalLength=sizeof(header)+slen_ho;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  HWSize_t totalLength=sizeof(header)+num_pos_ho*sizeof(docid_element_position);
  return totalLength;
}
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  if (res != 1)
    return 0;
  res=fwrite(wds->word, 1, strlen(wds->word), f);
  //printf("in memindex, hashtable written in bytes: %d\n",htlen_ho);
  //printf("bytes of the word written in %d, the length of the word:%d\n",res,strlen(wds->word));

  // Calculate and return the total amount of data written.
  // MISSING (fix this return value):
  int totalLength=sizeof(header)+strlen(wds->word)+htlen_ho;
  return totalLength;
}
...
static HWSize_t WriteHeader(FILE *f,
  ...
  if (fseek(f, sizeof(header), SEEK_SET) != 0)
    return 0;
  //const uint8_t *ptr=(const uint8_t *)malloc(doct_size);
  uint8_t *ptr=new uint8_t[doct_size];
  fread(ptr, doct_size, 1, f);
  uint8_t *origin_ptr=ptr;
  for(uint32_t i=0;i<doct_size;i++){
     crcobj.FoldByteIntoCRC(*ptr++);
  }
  //free(ptr);
  //std::cout<<"point 1"<<std::endl;
  delete [] origin_ptr;
  //ptr=(const uint8_t *)malloc(memidx_size);
  //std::cout<<"point 2"<<std::endl;
  ptr=new uint8_t[memidx_size];
  //std::cout<<"point 3"<<std::endl;
  fread(ptr, memidx_size, 1, f);
  origin_ptr=ptr;
  for(uint32_t i=0;i<memidx_size;i++){
     crcobj.FoldByteIntoCRC(*ptr++);
  }
  //free(ptr);
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  bucket_rec br;
  // MISSING:
  uint32_t numElements=NumElementsInLinkedList(li);
  br.chain_len=numElements;
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      element_position_rec elpr;
      elpr.element_position=nextelpos;
      elpr.toDiskFormat();
      //first write the element content into index file in the position of nextelpos
      LLIteratorGetPayload(it, (LLPayload_t *)&kv);
      ellen=fn(f, nextelpos, kv);
      res = fseek(f, offset, SEEK_SET);
      if (res != 0)
         return 0;
      if (fwrite(&elpr, sizeof(elpr), 1, f) != 1)
         return 0;
      //offset indicates the next postion in index file to write the element position record.
      offset+=sizeof(elpr);
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  //printf("how many hashtable buckets: %d\n",ht->num_buckets);
  for (i = 0; i < ht->num_buckets; i++) {
    // MISSING:
    LinkedList cur=ht->buckets[i];
    res=WriteBucketRecord(f,cur,next_bucket_rec_offset,next_bucket_offset);
    next_bucket_rec_offset+=res;
    if(NumElementsInLinkedList(cur)==0){
	continue;
    }
    res=WriteBucket(f,cur,next_bucket_offset,fn); 
